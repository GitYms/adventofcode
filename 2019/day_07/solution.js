...
const part1_test1 = [3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0]; // [4,3,2,1,0] -> 43210
const part1_test2 = [3,23,3,24,1002,24,10,24,1002,23,-1,23,
  101,5,23,23,1,24,23,23,4,23,99,0,0]; // [0,1,2,3,4] -> 54321
const part1_test3 = [3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,
  1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0]; // [1,0,4,3,2] -> 65210
const part2_test1 = [3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,
  27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5]; // [9,8,7,6,5] -> 139629729
const part2_test2 = [3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,
...
function processOutput2(data, input, i = 0) {
  console.log('i', i)
  const arr = [...data]
  const output = [];
  for(i; i < arr.length;) {
    ...
    if(opcode === 4) {
      ...
      i = i + 2;
      return processOutput2(arr, paramA, i);
    }
    if(opcode === 5) {
      paramA !== 0 ? i = paramB : i = i + 3;
    }
    ...
    if(opcode === 99) {
      console.log('when 99, output', output)
      return '';
    }
  }
}
processOutput2(part1_test1, 0, 0)
function getCircleSingal(sequences, data, init = 0) {
  // E->A->B->C->D->E 5个放大器
  // A input:[sequence[0], e]
  const outputArr = [init];
  while(sequences.length > 0) {
    const sequence = sequences.shift();
    const input = [sequence, outputArr[outputArr.length - 1]];
